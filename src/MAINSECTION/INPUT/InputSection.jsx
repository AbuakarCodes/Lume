...
function InputSection() {
  let {setMainInput , MainInput, setQuery, } = useMainContext()
      ...
      <div className="lg:max-w-[65%] pt-4 mx-auto flex gap-y-3  flex-col items-center ">
        <div className=" transition-all duration-300 flex items-center w-full h-[3.5rem] border border-gray-300 bg-[#F0F4F9] dark:bg-[#282A2C] rounded-[4rem] px-2 ">
          ...
          <input
            ...
            type="text"
            id="website-admin"
            className=" transition-all duration-300 flex-1 outline-none min-w-0 w-full h-full text-sm px-2.5 py-2 bg-[#F0F4F9] dark:bg-[#282A2C] text-gray-900 dark:text-white"
            placeholder="Ask from Conco Flow "
          />

          <button ref={sendbuttonRef}  onClick={QueryHandler} className="inline-flex items-center justify-center w-10 h-10 text-sm text-gray-900 dark:text-white hover:bg-[white]  dark:hover:bg-[#1B1C1D] transition-all duration-150 rounded-full">
